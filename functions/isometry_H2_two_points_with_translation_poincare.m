...
function f = isometry_H2_two_points_with_translation_poincare(p1, p2, q1, q2, translation_amount, orientation)
    ...
    L1 = isometry_H2_two_points_upper_half(z1, z2, w1, w2);
    len_side = distance_two_points(p1, p2);
    T = isometry_H2_two_points_upper_half(z1, z2, 1i, exp(len_side)*1i);
    trans_matrix = [exp(translation_amount/2), 0; 0, exp(-translation_amount/2)];
    
    L1M = [L1.a, L1.b; L1.c, L1.d];
    TM = [T.a, T.b; T.c, T.d];
    LM = L1M*inv(TM)*trans_matrix*TM;
