function draw_genus2_surface_separating_curve(ax)
...
%
% Optional argument ax: axes to draw on. If omitted, a new figure is created.
...
% --- 4. Render Surface ---
p = patch(ax, isosurface(x, y, z, V, 0));
...
while idx < size(C_z, 2)
    ...
    if any(xc < 1) && any(xc > 1)
        q1 = xc; q1(xc<1 | yc<0)  = NaN; plot3(ax, q1, yc, zc, 'Color', S.z0_Q1{1}, 'LineStyle', S.z0_Q1{2}, 'LineWidth', S.z0_Q1{3});
        q2 = xc; q2(xc<1 | yc>=0) = NaN; plot3(ax, q2, yc, zc, 'Color', S.z0_Q2{1}, 'LineStyle', S.z0_Q2{2}, 'LineWidth', S.z0_Q2{3});
        q3 = xc; q3(xc>=1 | yc<0) = NaN; plot3(ax, q3, yc, zc, 'Color', S.z0_Q3{1}, 'LineStyle', S.z0_Q3{2}, 'LineWidth', S.z0_Q3{3});
        q4 = xc; q4(xc>=1 | yc>=0)= NaN; plot3(ax, q4, yc, zc, 'Color', S.z0_Q4{1}, 'LineStyle', S.z0_Q4{2}, 'LineWidth', S.z0_Q4{3});
    else
        st = hole_styles{mod(k_h-1,2)+1};
        plot3(ax, xc, yc, zc, 'Color', st{1}, 'LineStyle', st{2}, 'LineWidth', st{3});
        k_h = k_h + 1;
    end
    idx = idx + n + 1;
end
...
while i_y < size(C_y, 2) && c_y < 2
    ...
    st = y_st{c_y+1};
    plot3(ax, xc_y, zeros(size(xc_y)), zc_y, 'Color', st{1}, 'LineStyle', st{2}, 'LineWidth', st{3});
    i_y = i_y + n + 1; c_y = c_y + 1;
end

% --- 7. Curve on Plane x = 1 ---
c_x1 = contourslice(ax, x, y, z, V, 1, [], [], [0 0]);
set(c_x1, 'EdgeColor', S.x1_C{1}, 'LineStyle', S.x1_C{2}, 'LineWidth', S.x1_C{3});

daspect(ax, [1 1 1]); view(ax, -55, 30); camlight(ax); lighting(ax, 'gouraud'); grid(ax, 'on');
title(ax, 'Genus 2 Surface: with separating curve');
